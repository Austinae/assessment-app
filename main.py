...
class CreateSummative(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        self.activatePage.config(font=("Courier", 25))
        self.activatePage.place(relx=0.415, rely=0.3)
...
class CreateFormative(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        bg_image = tk.PhotoImage(file="bg7.png")
        bg_label = tk.Label(self, image=bg_image)
        bg_label.image = bg_image
        bg_label.place(relwidth=1, relheight=1)
        ...
        button1 = tk.Button(self, text="Back", background="MistyRose4",
                            command=lambda: controller.show_frame(DashboardLec))
        button1.config(font=("Courier", 25))
        button1.place(relx=0.4, rely=0.7)

        button2 = tk.Button(self, background='turquoise', font=('courrier', 10), text="Create Formative Test", command=lambda: self.createWindow2())
        ...
        self.activatePage = tk.Button(self, text="Load Tests", command=lambda: self.user(), background='orchid4')
        self.activatePage.config(font=("Courier", 25))
        self.activatePage.place(relx=0.35, rely=0.2)
...
class DetailedResults(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        bg_image = tk.PhotoImage(file="bg6.png")
        bg_label = tk.Label(self, image=bg_image)
        bg_label.image = bg_image
        bg_label.place(relwidth=1, relheight=1)

        label = tk.Label(self, text="Detailed results", background='turquoise', font=LARGE_FONT)
        label.place(relx=0.44, rely=0.1)
        ...
        self.load = tk.Button(self, text="Load Tests", background='red2', command=lambda: self.viewTests())
        self.load.config(font=("Courier", 25))
        self.load.place(relx=0.4, rely=0.2)
    ...
    def viewTests(self):
        ...
        for count, test in enumerate(self.tests_s):
            ...
            if a < b:
                counter += 0.04
                tk.Button(self, text=test, font=("Courier", 8), background='light coral',
                           command=lambda count=count: self.viewResults_s(count)).place(relx=0.4, rely=counter)
        csv_file.close()
        ...
        for count, test in enumerate(self.tests_f):
            ...
            if a < b:
                counter += 0.04
                tk.Button(self, text=test, font=("Courier", 8), background='light coral',
                           command=lambda count=count: self.viewResults_f(count)).place(relx=0.4, rely=counter)
        csv_file.close()
    ...
    def viewResults_s(self, index):
        ...
        try:
            ...
            tm.showinfo("Error","No results are available")
